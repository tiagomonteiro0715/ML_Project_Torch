"""
LRA Benchmark — Pathfinder: Visual path detection in synthetic images.
Can the model trace a curvy line through noise to see if two dots are
connected?  (32×32 images → 1024 sequence length)

Generates: pathfinder_lra.pt
"""
...
if __name__ == "__main__":
    ...
    gen = PathfinderGenerator(
        resolution=32, seed=42, device=device,
        contour_length=14, num_distractor_snakes=6,
    )
    ...
    train_tokens, train_labels = gen.generate(160000, batch_size=batch_size_pf)
    val_tokens, val_labels = gen.generate(20000, batch_size=batch_size_pf)
    test_tokens, test_labels = gen.generate(20000, batch_size=batch_size_pf)
